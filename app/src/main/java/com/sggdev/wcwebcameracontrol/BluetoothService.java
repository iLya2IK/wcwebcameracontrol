...
public class BluetoothService extends Service {
    ...
    private String mAutoSendWriteChar;
    private String mAutoSendCommand;
    private int mAutoSendValue = 0;
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
            ...
            if (status == GATT_SUCCESS) {
                ...
                if (descriptor.getUuid().equals(SampleGattAttributes.CLIENT_CHARACTERISTIC_CONFIG)) {
                    ...
                    if (data != null && data.length > 0) {
                        if (data[0] > 0) {
                            broadcastUpdate(ACTION_NOTIFICATION_GRANTED, characteristic);
                        } else {
                            broadcastUpdate(ACTION_NOTIFICATION_CANCELED, characteristic);
                        }
                    }
                }
            }
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            ...
            if (newState == BluetoothProfile.STATE_CONNECTED) {
                if (mBluetoothGatt != gatt) mBluetoothGatt = gatt;
            ...
            if (mAutoSendMode) {
                if (mAutoSendValue == 1) {
                    if (status == GATT_SUCCESS) {
                        ...
                        if (value.equals(mAutoSendCommand)) {
                            mAutoSendValue = 2;
                            sendData("*cmd* exit\r\n");
                            broadcastUpdate(ACTION_SENDING_SUCCEED);
                        }
                    }
                } else {
                    disconnect();
        ...
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {
            if (status == GATT_SUCCESS) {
                mNotificationCharacteristics.clear();
                    ...
                    for (BluetoothGattCharacteristic characteristic : gattService.getCharacteristics()) {
                        ...
                        if (mAutoSendMode) {
                            ...
                            if (mid.equals(mAutoSendWriteChar)) {
                                ...
                                mAutoSendValue = 1;
                                sendData(mAutoSendCommand);
                                return;
                            }
                        } else {
                            if (SampleGattAttributes.isInputChar(characteristic.getUuid().toString())) {
                                mWriteCharacteristic = characteristic;
                                broadcastUpdate(ACTION_INPUT_CHARACTERISTIC_DISCOVERED);
                            }
                            if (SampleGattAttributes.isOutputChar(characteristic.getUuid().toString())) {
    ...
    private void descriptorUpdate(final String action,
    ...
    public ArrayList<BluetoothGattCharacteristic> getNotifyCharacteristics() {
